...
const db = mysql.createConnection({
    host: 'localhost',
    user: 'root',
    password: 'Binoza2610', // ⚠️ Cek Password
    database: 'db_resep'
});
...
app.post('/auth/register', (req, res) => {
    ...
    [username, email, password], (err, result) => {
        ...
        db.query("INSERT INTO api_keys (user_id, api_key, name) VALUES (?, ?, ?)", [result.insertId, firstKey, 'Default Key']);
...
// ===== DASHBOARD & API KEY =====
app.get('/my-keys', (req, res) => {
    if (!req.session.user) return res.status(401).json({ error: 'Login required' });
    db.query("SELECT * FROM api_keys WHERE user_id = ? ORDER BY created_at DESC", [req.session.user.id], (err, results) => res.json({ keys: results }));
});
...
// 4. EDIT (FULL UPDATE dengan FOTO)
app.put('/admin/api/resep/:id', upload.single('foto'), (req, res) => {
    ...
    if (req.file) {
        // Kalau ada foto baru, update semua kolom termasuk gambar
